...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let submitCommentCreateForm = form => {
        ...
        fetch(url, {
            ...
            if (formButton.getAttribute('value') === 'parent') {
                ...
                if (replyNum > 1) {
                    reply.textContent = gettext("Ответов");
                } else {
                    reply.textContent = gettext("Ответ");
                }
                commentCount(1);
                // update followBtn
            ...
            Swal.fire({
                icon: 'error',
                title: 'Упс...',
                text: 'Ваш комментарий не был опубликован. Попробуйте еще раз.'
            })
        });
    };
    ...
    let submitEditCommentForm = form => {
        ...
        fetch(url, {
            ...
            Swal.fire({
                icon: 'error',
                title: 'Упс...',
                text: 'Комментарий не был изменен. Попробуйте еще раз.'
            })
    ...
    let loadForm = deleteBtn => {
        ...
        fetch(url, {headers: headers}).then(response => {
            ...
            Swal.fire({
                icon: 'error',
                title: 'Упс...',
                text: 'Комментарий не был удален. Попробуйте еще раз.'
            })
    ...
    let submitDeleteCommentForm = form => {
        ...
        fetch(url, {
            ...
            if (isParent) {
                ...
                if (replyNum > 1) {
                    replyLinkElement.textContent = gettext("Ответов");
                } else {
                    replyLinkElement.textContent = gettext("Ответ");
                }
    ...
    let submitReaction = reactionButton => {
        ...
        fetch(url, {
            ...
            Swal.fire({
                icon: 'error',
                title: 'Упс...',
                text: 'Реакция на комментарий не была опубликована. Попробуйте еще раз.'
            })
    ...
    let toggleFollow = (followButton, form) => {
        ...
        fetch(url, {
            ...
            Swal.fire({
                icon: 'error',
                title: 'Упс...',
                text: 'Подписка не была оформлена. Попробуйте еще раз.'
            })
    ...
    let submitFlagForm = (flagButton, reason = null, info = null) => {
        ...
        fetch(url, {
            ...
            Swal.fire({
                icon: 'error',
                title: 'Упс...',
                text: 'Комментарий не был отмечен. Попробуйте еще раз.'
            })
    ...
    const toggleText = readMoreButton => {
        ...
        if (readMoreButton.previousElementSibling.classList.contains('d-none')) {
            readMoreButton.innerHTML = gettext("...развернуть");
        } else {
            readMoreButton.innerHTML = gettext("свернуть");
        }
    };
